...
#: ДЛИНА ТАБЛИЦЫ. Столько же, сколько гнёзд локаций (0x37 в worldmap.py):
#: движок и не спрашивает свет у карты с бОльшим номером. Сразу за таблицей
#: лежит чужая, из указателей, и её значения на цвет не похожи только на вид:
#: у карты 55 там 0x00451C4A, у 56 — 0x00451C4B, дальше подряд 0x00451C51,
#: 0x00451C56 — это адреса, растущие на длину строки, а не уровни света.
#:
#: Без этой границы номер за пределами канона читает соседнюю таблицу и
#: получает «постоянный свет» из указателя: карта 156 брала 0x00451F26, то
#: есть синий 0x26=38, зелёный 0x1F=31, красный 0x45=69 — красного вдвое
#: больше зелёного, и вся сцена заливалась розовым. Ноль здесь значит
#: «суточный цикл», как у обычной карты.
FIXED_LIGHT_COUNT = 0x37


def fixed_light_word(number):
    """Сырая запись таблицы 0x4617B0 для карты; вне таблицы — ноль."""
    from .exetables import va_to_foff
    from .paths import game_file
    if not 0 <= number < FIXED_LIGHT_COUNT:
        return 0
