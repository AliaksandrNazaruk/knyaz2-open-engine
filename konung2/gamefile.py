...
def ground_items(number: int, world: int = 0, clock: int = 0) -> list[dict]:
    ...
    for index in range(GROUND_ITEMS_COUNT):
        ...
        # Деньги движок берёт ПО МОДУЛЮ (VA 0x4115DE зовёт abs), поэтому
        # отрицательная запись — это тоже монеты, а не долг.
        money = abs(struct.unpack_from("<h", record, GROUND_MONEY_AT)[0])
...
T_UNITS = RecordTable('units', 0x46322, 2000, 256, [
    ...
    ('kind',     0x17, 'u8'),     # 3/0x0B/0x0C — не ставятся в клетку сетки
    ('flags',    0x1A, 'u8'),
    ...
    for party in range(T_PARTIES.count):
        ...
        for step in range(count):
            ...
            units.append({
                ...
                "side": unit[0x1B], "kind": unit[0x17], "flags": unit[0x1A],
...
def map_exits(number: int, world: int = 0) -> list[dict]:
    ...
    for index in range(T_EXITS.count):
        ...
        exits.append({
            ...
            "to_name": (places[target] if 0 <= target < len(places) else
                        ("глобальная карта" if target == EXIT_LEAVE else "особый переход")),
            "facing": record[2],
            "entry_row": struct.unpack_from("<H", record, 5)[0],
            "entry_col": struct.unpack_from("<H", record, 7)[0],
