...
class Bounds:
    ...
    def sort_key(self, position: Point) -> int:
        """Ключ глубины объекта: ПОДОШВА ЕГО КАДРА минус четверть высоты.

        Дословно из прохода сцены (VA 0x004267b8:204-217)::

            iVar9   = камера + (заголовок[состояние*2 + 0x30] >> 16) + объект_y
            local_2c = max(высота main, высота стен)
            ключ    = iVar9 + local_2c;   если флаги & 8 — минус local_2c >> 2

        Слагаемое из заголовка — это `dy` якоря кадра (``res.py``,
        ``SIMPLE_ANCHORS_AT = 0xC0``: дword по 0xC0, старшая половина), то
        есть наш ``offset_y``. Поэтому ключ равен нижнему краю кадра, а
        четверть поднимает линию к подошве передней стены.

        СМЕЩЕНИЕ КАДРА ОТСЮДА УБИРАТЬ НЕЛЬЗЯ. Один раз убрали — линия глубины
        уехала вниз на 183…450 точек, и всё, что стоит перед домом или
        столбом, стало тонуть в его спрайте. «Живой замер», которым это тогда
        подтвердили, считал ключ ТОЙ ЖЕ формулой с обеих сторон и потому
        сходился всегда. Проверять надо картинкой или формулой из
        декомпилята, а не своей же арифметикой.

        Разбор целиком — ``docs/RENDER_DEPTH.md``.
        """
