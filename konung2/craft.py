...
#: ТАЙНИКИ. Спрятанную кучу (знак поля +0x0F её записи) достают двумя
#: вещами, и обе разбирает FUN_00436C48 по классу:
#:
#:   32 «Лопата», случай ' ' — вещь НЕ тратится, движок лишь переводит
#:      курсор в режим копки (_DAT_00849650 = 5) и играет звук 0x12; дальше
#:      щелчок по любой клетке шлёт туда приказ 3 «обыскать» (VA 0x4217B0),
#:      и видеть кучу при этом не нужно;
#:   35 «Медное зеркало колдуна», случай '#' — у всех спрятанных куч ТЕКУЩЕЙ
#:      карты знак переворачивается, и они оказываются на виду; вещь при
#:      этом ТРАТИТСЯ (`*param_3 = -1` в хвосте случая).
#:
#: Проверку «есть ли Лопата» движок делает перебором мешка по классу —
#: FUN_00434F8C(0x20) из ветки обыска VA 0x4115AC.
SHOVEL_CLASS, MIRROR_CLASS = 32, 35
DIG_CURSOR, DIG_ORDER = 5, 3
...
def rules() -> dict:
    ...
    return {
        ...
        # тайники: чем достают спрятанную кучу (VA 0x436C48, 0x4115AC)
        "caches": {
            "shovel_class": SHOVEL_CLASS,
            "mirror_class": MIRROR_CLASS,
            "dig_cursor": DIG_CURSOR,
            "dig_order": DIG_ORDER,
            # лопата остаётся в мешке, зеркало тратится
            "shovel_spent": False,
            "mirror_spent": True,
        },
