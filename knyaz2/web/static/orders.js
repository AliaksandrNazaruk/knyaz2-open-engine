...
export function orderWaitTalk(unit) {
  ...
  unit.goal = null;
  unit.workRest = 0;
  // РАЗГОВОР УБИРАЕТ ОРУЖИЕ (VA 0x410DCA): двумя строками ниже записи
  // приказа 0x0C стоит `and byte [eax+0x19], 0xfb`. Зверя не трогают
  // (0x410DC1: `test +0x1A, 0x40`).
  //
  // Это ОДНО из четырёх снятий, а не единственное: остальные три сделаны
  // через `xor 4` — 0x41145F (рассудок, когда дел не осталось; там же
  // юнит встаёт в блок 16) и 0x420577, 0x420D0F. Искать такие вещи надо
  // всеми формами сразу: по одной я насчитал единственное снятие и на
  // ровном месте объявил каноном «житель уходит из казармы с мечом».
...
//
// ГОЛОС ОТКЛИКА — ТОЛЬКО У ЩЕЛЧКА. В движке «кого показывает панель»
// (`_DAT_00849514`) и «кто выбран» — два разных дела, и обыск кучи ставит
// ПЕРВОЕ, минуя выбор: `FUN_004115AC` пишет обыскивающего прямо в
// `_DAT_00849514` и зовёт `FUN_00424128`, который только перекладывает
// мешок в столбец обмена. Ни 0x423F80, ни проигрывателя голоса в той
// ветке нет — скан вызовов герметичен. У нас панель и обмен идут за
// первым выбранным, поэтому обыск выбор всё-таки ставит; чтобы он при
// этом не оговаривался, зовущий просит `silent`.
export function select(unit, add = false, { silent = false } = {}) {
  if (!unit) return selection;
  if (!add) {
    selection.length = 0;
    selection.push(unit);
    if (!silent) sfxSelect(unit);   // голос отклика (0x421690 -> 0x42D308)
    return selection;
  }
  ...
  if (at >= 0) deselect(unit);
    ...
    selection.push(unit);
    sortSelection();
    if (!silent) sfxSelect(unit);   // пополнение списка тоже звучит (0x423F80)
  }
  return selection;
}
...
export function orderUnit(unit, row, col, kind, target = null) {
  ...
  //
  // И ТОЛЬКО СВОИМ. Оба движковых места, где бит ставится, перебирают
  // выбранных игроком (массив 0x840B94) — чужого юнита он не касается
  // никогда, а сама выдача приказа 0x416574 в +0x19 пишет лишь бит 0x80.
  // Здесь бит ставился ВСЕМ подряд, и деревенские, получив наряд по
  // хозяйству (units.js, работа), латали его себе тоже; беды не было ровно
  // потому, что завершение приказа его тут же и снимало. Теперь снятие
  // канонное — значит и постановка обязана быть канонной, иначе житель,
  // сходив за дровами, навсегда перестал бы видеть врага.
  const kinds = orderKinds();
  const ofPlayer = Boolean(unit.ally) || unit === fallbackUnit;
  if (ofPlayer) unit.busy = kind !== kinds.target;
...
// Завершение приказа (VA 0x416E24) трогает РОВНО два поля: цель +0x10 и байт
// приказа +0x16 по маске 0xB0. Байт состояния +0x19 функция не касается — а
// «занят приказом» живёт именно там.
//
// ЗДЕСЬ СТОЯЛО `unit.busy = false` — подмена одного байта другим. Маска 0xB0
// правда гасит бит 0x40, но бит 0x40 БАЙТА ПРИКАЗА, а не байта состояния.
// Ценой подмены был весь боевой быт: герой, дойдя куда послали, тем же тактом
// выходил из-под руки игрока, рассудок (0x4111E8 — вся ветка под
// `(+0x19 & 0x40) == 0`) заново брал ближайшего врага и гнал героя на него.
// Приходилось щёлкать без остановки, чтобы просто стоять на месте.
