...
async function prepare() {
  ...
  await new Promise((готово, худо) => {
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = "/menu.css";
    link.addEventListener("load", готово, { once: true });
    link.addEventListener("error", худо, { once: true });
    document.head.append(link);
  });
  node.append(stage);
  document.addEventListener("menu:continue", () => gameMenuShow(false));
  document.addEventListener("menu:save", (событие) => {
    const ок = world.onMenuSave?.();
    if (событие.detail) событие.detail.ok = ок;
  });
...
export function gameMenuShow(on = true) {
  ...
  if (on && !ready) {
    ready = prepare().catch((беда) => {
      console.warn("меню не втянулось:", беда);
      ready = null;
  ...
  if (!on && ready) view.follow = settingsLoad().follow;
  open = Boolean(on);
  node.hidden = !open;
